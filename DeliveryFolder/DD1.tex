...
In this section it is predented an overall descripition of the most relevant algorithm for the application, explained using pseudocode notation. 

\subsection{Run Placement Algorithm}
This algoritm controls if it possible to create a run in a certain moment and location, checking if there are some other runs that prevent to do it. When a user tries to create a run there are 4 possible situation:
\begin{itemize}
...
together in order for a project to be developed in a correct way. It will be presented
in the paragraphs below a plan for the implementation, integration, and testing of
the different components that, together, form our system.
...
The second and third approaches imply that a component will be implemented
and tested without the need for the components that this depends on to be imple-
mented and tested already. Although this might look like a good idea, it carries a
...
first ones to be developed and extensively tested, followed by the ones that only de-
pend on those, until we reach the highest layer of the system, with the components
...
\subsection{DBMS}
The first component to be implemented is the DBMS. It shall be implmented and then fully tested before finishing the implementation of the layer above since it has
to be configured and operative in order to allow to test all of the components which
need access to the database.
\subsection{Server}
The next set of components to be implemented and tested are the ones that belong
to the Business Layer, which encapsulates the business logic of the system. This layer is composed of the server component and all its sub-components.
...
After this,and following a critical-module-first approach, we should develop the HealthDataController , RunController, AccountController and the DataAccessController.Both of this controllers require the interaction with the external services to be fully functional. The last sub-components of the server to be developed will be the router.After being fully implemented, the Server component must be integrated with
the DBMS component, as it needs the DBMS to work properly.
Before integration testing of the two integrated components, the used external
...
\begin{tabular}{ |p{0.25\textwidth}|p{0.4\textwidth}|p{0.25\textwidth}| } 
 ...
 \textbf{DATE} & \textbf{TASK} & \textbf{HOURS} \\ 
  ...
  \hline
  1/12/2018 & Implementation,Integration and test plan & 1.5\\ 
   \hline
  3/12/2018 & Requirements traceability & 2 \\ 
  \hline
  \textbf{TOTAL} & \multicolumn{2}{c|}{} \\ 
  \hline
\end{tabular}
